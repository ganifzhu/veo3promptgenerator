document.addEventListener('DOMContentLoaded', () => {
    ...
    function handleSaveToFile() {
        ...
        try {
            ...
            a.href = url;
            const fileName = `${story.projectTitle.replace(/ /g, '_')}.veo.encrypted`;
            a.download = fileName;
    ...
    function handleLoadFromFile(event) {
        ...
        reader.onload = (e) => {
            try {
                ...
                const bytes = CryptoJS.AES.decrypt(encryptedData, SECRET_KEY);
                const decryptedJsonString = bytes.toString(CryptoJS.enc.Utf8);
                if (!decryptedJsonString) { throw new Error("Gagal mendekripsi file."); }
    ...
    function setupCopyButton(button, sourceElement) {
        ...
        button.addEventListener('click', () => {
            const textToCopy = sourceElement.value || sourceElement.innerText;
